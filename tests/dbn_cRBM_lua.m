...
%% Sample from p(y | h)
y_sample = zeros(n_classes,1);
sumsofar = 0;
r=rbm.rand([1,1]);
done=0;
for y_=0:(n_classes-1)  %iterate over classes
...
% p(y | x)
Wx = W*x;
p_y_given_x = zeros(n_classes,1);
for y_ = 0:(n_classes-1)
